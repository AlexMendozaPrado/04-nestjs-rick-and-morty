...
export default function Header({ esOscuroTemas, setIsOscuroTemas }: Headerprops) {
          ...
          <div className="flex justify-between items-center w-full max-w-4xl px-2 gap-2">
            ...
            <div className="flex justify-between max-w-[130px] w-full items-center gap-2">
              ...
              <label className="relative inline-block w-16 h-8 cursor-pointer">
                ...
                />
                <span className={`slider block absolute inset-0 bg-blue-500 rounded-full transition-all duration-300 ${esOscuroTemas ? 'bg-blue-600' : 'bg-blue-500'}`}>
                  <span className={`absolute left-1 bottom-1 bg-gray-100 w-7 h-7 rounded-full transform transition-all duration-300 ${esOscuroTemas ? 'translate-x-7' : ''}`}></span>
                </span>
              </label>
            </div>
          </div>
